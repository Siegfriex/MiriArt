...
Socket Factory 의존성이 반영된 새 이미지를 빌드한 뒤, **Cloud SQL 인스턴스 연결**과 **VPC 커넥터**(Redis 접근용)를 넣어 배포합니다.
...
# 1) 로컬에서 이미지 빌드 후 푸시했거나, Cloud Build로 빌드했다고 가정
# 2) Cloud Run 배포
...
- `--vpc-connector` / `--vpc-egress`: Redis(10.15.105.203) 등 VPC 내부 접근용. 커넥터 미생성 시 3번 먼저 실행.
