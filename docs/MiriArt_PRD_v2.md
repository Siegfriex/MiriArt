# MiriArt PRD v2.0 — 제품 요구 명세서

> **목적**: Legacy PRD v1.1 갭 메우기 + 현 구현 상태 반영 + 커뮤니티 비전 통합
> **버전**: 2.0 | **작성일**: 2026-02-22
> **기반**: Legacy PRD v1.1, FSD v2.0, ERD v2.0, Community Design v1.0, FE Disassembly Report v1.0
...
### 0.2 Legacy PRD v1.1 vs MiriArt PRD v2.0 변경점

| 항목 | Legacy PRD v1.1 | MiriArt PRD v2.0 |
|------|-----------------|-------------------|
...
### 4.1 현 구현 갭 분석 (Legacy PRD vs MiriArt 02_21dys)

| 기능 | Legacy PRD 명세 | 현 구현 상태 | 갭 |
|------|-----------------|-------------|-----|
| 회원가입/로그인 | 이메일+비밀번호 F1 | Mock (navigate만) | Auth API 미구현 |
| AI 분석 | F3 Layer 1 | ApiService.analyze (Mock 데이터) | FastAPI 미연동 |
| AI 채팅 | F5 Layer 3 | ApiService.chat (Cloud Run 직접) | BE 프록시 미구현 |
| 분석 결과 저장 | analyses 테이블 | MOCK_ARTWORKS | DB 미연동 |
| 합격 확률 | F4 Layer 2 | MOCK_TIERS (하드코딩) | Theory Engine 미구현 |
| 크레딧/구독 | F7 | SubscriptionSheet UI만 | 결제 미구현 |
| 커뮤니티 | 없음 (Legacy) | 없음 | 신규 설계 |

### 4.2 핵심 엔티티 요약

| 엔티티 | Phase | 설명 |
|--------|-------|------|
| `User` | P1 | 소셜 로그인 기반 계정 |
| `Analysis` | P1 | 작품 분석 결과 (5축 + fixScope + 합격 확률) |
| `Plan` | P1 | 구독 플랜 (FREE/BASIC/PREMIUM) |
| `AnalysisUsageLog` | P1 | 월별 분석 카운트 |
| `ChatSession` | P2 | AI 채팅 세션 (P1: Redis) |
| `ChatMessage` | P2 | AI 채팅 메시지 (P1: Redis) |
| `Post` | C1 | 커뮤니티 게시글 (free/qna) |
| `Answer` | C1 | Q&A 답변 |
| `Persona` | C1 | 가명 시스템 |
| `ReputationLedger` | C1 | 평판 포인트 이력 |
...
| 문서명 | 버전 | 위치 |
|--------|------|------|
| MiriArt FSD v2.0 | 2.0 | `docs/02_21dys_FSD_v2.md` |
| MiriArt ERD v2.0 | 2.0 | `docs/02_21dys_ERD_v2.md` |
| MiriArt API Contract | 1.0 | `docs/02_21dys_API_CONTRACT.md` |
| MiriArt BE Setup Guide | 1.0 | `docs/02_21dys_BE_SETUP_GUIDE.md` |
