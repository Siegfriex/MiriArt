...
const TIER_STYLES: Record<ComparisonTier['level'], {
  ...
  MID:      { bg: 'bg-white/5 border-white/10',                 bar: 'bg-white/30',        badge: 'bg-white/10 text-text-mid',                   badgeText: 'MID' },
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <div className="text-sm text-white font-medium mt-0.5">{TIER_LABELS[tier.level]}</div>
          </div>
        </div>
              ...
              {tier.items.map((item, idx) => (
                <div key={idx} className="pt-3 flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="text-xs font-medium text-white">{item.university}</div>
  ...
  return (
    <section>
      <div className="flex justify-between items-center mb-4">
        <H2 className="text-white">{STRINGS.RESULT_COMPARISON_TITLE}</H2>
      ...
      <div className="relative">
        ...
        {/* 합격작 미업로드 시 잠금 오버레이 */}
        {!hasAcceptedArtwork && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-dark-900/60 rounded-xl backdrop-blur-sm">
            <div className="w-12 h-12 bg-dark-800 rounded-full flex items-center justify-center mb-3 border border-white/10">
              <Lock size={20} className="text-text-mid" />
            </div>
            <H3 className="text-white text-center text-sm">{STRINGS.RESULT_COMPARISON_LOCKED}</H3>
          </div>
        )}
      </div>
    </section>
